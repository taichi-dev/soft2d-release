...
public:
  constexpr bool is_valid() const { return instance_ != VK_NULL_HANDLE; }
  void destroy();
...
#endif // TI_AOT_DEMO_WITH_GLFW
  // FIXME: (penguinliong) This one is somehow deprecaeted so please simply
  // don't use it.
  ...
  constexpr VmaAllocator vma_allocator() const { return vma_allocator_; }

  // The renderer's representation as Taichi objects.
  constexpr TiArch arch() const { return TI_ARCH_VULKAN; }
  constexpr PFN_vkGetInstanceProcAddr loader() const { return loader_; }
  constexpr TiRuntime runtime() const { return runtime_; }
...
enum PrimitiveTopology {
  ...
  L_PRIMITIVE_TOPOLOGY_LINE_LIST,
  L_PRIMITIVE_TOPOLOGY_LINE_STRIP,
...
public:
  constexpr bool is_valid() const { return pipeline_ != VK_NULL_HANDLE; }
  void destroy();
