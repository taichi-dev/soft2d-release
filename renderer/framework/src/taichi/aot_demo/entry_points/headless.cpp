...
void initialize(const char *app_name, int argc, const char **argv) {
  ...
  args::reg_arg<args::UintParser>(
      "-f", "--frames", CFG.frame_count,
...
void save_framebuffer_to_bmp(const ti::NdArray<uint8_t> &framebuffer,
                             uint32_t i) {
  std::string index = std::to_string(i);
  std::string zero_padding(4 - index.size(), '0');
  std::string path =
      CFG.output_prefix + zero_padding + std::to_string(i) + ".bmp";
  liong::util::save_bmp((const uint32_t *)framebuffer.map(),
                        framebuffer.shape().dims[0],
                        framebuffer.shape().dims[1], path.c_str());
  framebuffer.unmap();
}
