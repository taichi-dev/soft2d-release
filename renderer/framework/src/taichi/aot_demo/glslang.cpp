...
struct glslang_lifetime_t {
  glslang_lifetime_t() { glslang_initialize_process(); }
  ~glslang_lifetime_t() { glslang_finalize_process(); }
} LIFETIME;
...
std::vector<uint32_t> glsl2spv(glslang_stage_t stage, const std::string &glsl) {
  ...
  std::vector<uint32_t> outShaderModule(
      glslang_program_SPIRV_get_size(program));
