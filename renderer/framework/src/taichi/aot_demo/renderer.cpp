...
Renderer::Renderer(bool debug, uint32_t width, uint32_t height) {
  ...
  if (debug) {
    ...
    res = vkCreateDebugUtilsMessengerEXT_(instance, &dumci, nullptr,
...
try_another_physical_device:
  ...
  // (penguinliong) Try not to be trapped by Intel's garbage integrated GPU or
  // CPU software rasterizers.
  if (pdp.deviceType == VK_PHYSICAL_DEVICE_TYPE_INTEGRATED_GPU ||
      pdp.deviceType == VK_PHYSICAL_DEVICE_TYPE_CPU) {
  ...
  uint32_t ndep = 0;
  vkEnumerateDeviceExtensionProperties(physical_device, nullptr, &ndep,
                                       nullptr);
  std::vector<VkExtensionProperties> deps(ndep);
  res = vkEnumerateDeviceExtensionProperties(physical_device, nullptr, &ndep,
                                             deps.data());
  check_vulkan_result(res);
  ...
  bool has_pdp2 =
      is_vk_1_1 ||
      has_dep(VK_KHR_GET_PHYSICAL_DEVICE_PROPERTIES_2_EXTENSION_NAME);
  ...
  for (uint32_t i = 0; i < nqfp; ++i) {
    ...
    if ((qfps.at(i).queueFlags & qfs) == qfs) {
  ...
  ti::NdArray<float> rect_vertex_buffer{};
  {
    ...
    rect_vertex_buffer =
        ti::NdArray<float>(std::move(memory), std::move(ndarray));
  ...
  {
    std::vector<glm::vec2> data{
        glm::vec2(0.0, 1.0), glm::vec2(0.0, 0.0), glm::vec2(1.0, 0.0),
        glm::vec2(0.0, 1.0), glm::vec2(1.0, 0.0), glm::vec2(1.0, 1.0),
    };
...
void Renderer::destroy() {
  ...
  vmaDestroyImage(vma_allocator_, depth_attachment_,
                  depth_attachment_allocation_);
  vmaDestroyImage(vma_allocator_, color_attachment_,
                  color_attachment_allocation_);
  ...
  if (debug_utils_messenger_ != VK_NULL_HANDLE) {
    ...
    vkDestroyDebugUtilsMessengerEXT_(instance_, debug_utils_messenger_,
                                     nullptr);
  }
...
void Renderer::set_surface_window(GLFWwindow *window) {
  ...
  res =
      vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physical_device_, surface, &sc);
  check_vulkan_result(res);
...
void Renderer::set_framebuffer_size(uint32_t width, uint32_t height) {
  ...
  res = vmaCreateImage(vma_allocator_, &caci, &aci, &color_attachment,
                       &color_attachment_allocation, nullptr);
  check_vulkan_result(res);
  ...
  res = vmaCreateImage(vma_allocator_, &daci, &aci, &depth_attachment,
                       &depth_attachment_allocation, nullptr);
  check_vulkan_result(res);
...
void Renderer::begin_render() {
  ...
  vkCmdPipelineBarrier(command_buffer, VK_PIPELINE_STAGE_ALL_COMMANDS_BIT,
                       ...
                       nullptr, (uint32_t)imbs.size(), imbs.data());
...
void Renderer::enqueue_graphics_task(const GraphicsTask &graphics_task) {
  ...
  vkCmdBindDescriptorSets(frame_command_buffer_,
...

  {
    const TiVulkanMemoryInteropInfo &vmii =
        export_ti_memory(config.vertex_buffer);
    VkDeviceSize o = 0;
  ...
  if (is_indexed) {
    const TiVulkanMemoryInteropInfo &vmii =
        export_ti_memory(config.index_buffer);
    vkCmdBindIndexBuffer(frame_command_buffer_, vmii.buffer, 0,
                         VK_INDEX_TYPE_UINT32);

    vkCmdDrawIndexed(frame_command_buffer_, config.index_count,
                     config.instance_count, 0, 0, 0);
  } else {
    vkCmdDraw(frame_command_buffer_, config.vertex_count, config.instance_count,
              0, 0);
  }
}
...
void Renderer::present_to_surface() {
  ...
  uint32_t i = !0u;
  res = vkAcquireNextImageKHR(device_, swapchain_, 0, VK_NULL_HANDLE,
                              acquire_fence_, &i);
  check_vulkan_result(res);
...
void Renderer::present_to_ndarray(ti::NdArray<uint8_t> &dst) {
  ...
  vkCmdCopyImageToBuffer(command_buffer, color_attachment_,
                         VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL, vmii.buffer, 1,
                         &bic);
...
const TiVulkanMemoryInteropInfo &Renderer::export_ti_memory(TiMemory memory) {
  ...
  if (it == ti_memory_interops_.end()) {
    ...
    it = ti_memory_interops_.emplace(std::make_pair(memory, std::move(vmii)))
             .first;
  }
  return it->second;
}
...
GraphicsTask::GraphicsTask(const std::shared_ptr<Renderer> &renderer,
  ...
  res = vkCreateDescriptorSetLayout(device, &dslci, nullptr,
  ...
  for (size_t i = 0; i < config.resources.size(); ++i) {
    ...
    case L_GRAPHICS_TASK_RESOURCE_TYPE_TEXTURE: {
      ...
      ti_export_vulkan_image(renderer_->runtime(), resource.texture.image,
                             &viii);
      check_taichi_error();
  ...
  vkUpdateDescriptorSets(device, (uint32_t)wdss.size(), wdss.data(), 0,
                         nullptr);
  ...
  case L_PRIMITIVE_TOPOLOGY_POINT:
    ...
    break;
  case L_PRIMITIVE_TOPOLOGY_LINE_LIST:
    pt = VK_PRIMITIVE_TOPOLOGY_LINE_LIST;
    break;
  case L_PRIMITIVE_TOPOLOGY_LINE_STRIP:
    pt = VK_PRIMITIVE_TOPOLOGY_LINE_STRIP;
    break;
  ...
  res =
      vkCreateGraphicsPipelines(device, nullptr, 1, &gpci, nullptr, &pipeline);
  check_vulkan_result(res);
