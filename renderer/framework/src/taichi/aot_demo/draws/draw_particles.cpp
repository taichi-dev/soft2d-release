...
std::unique_ptr<GraphicsTask> DrawParticlesBuilder::build() {
  ...
  case 2:
    ...
    vertex_get =
        "vec4(pos.x * 2.0 - 1.0, (1.0 - pos.y) * 2.0 - 1.0, 0.0f, 1.0f)";
    break;
  case 3:
  ...
  default:
    throw std::logic_error(
        "vertex position can only `float`, `vec2`, `vec3` or `vec4`");
  }
  ...
  std::string vert;
  {
    ...
    ss << R"(
      ...
      layout(location=1) out vec4 v_normal;)"
       << vertex_declr << uniform_buffer_declr << color_buffer_declr << R"(
      void main() {
        gl_Position = u.view2clip * u.world2view * u.model2world * )"
       << vertex_get << R"(;
        v_color = )"
       << color_get << R"(;
      }
