...
struct BackgroundGrid : public App {
  ...
  std::unique_ptr<GraphicsTask> draw_grid_nodes;
  ...
  std::unique_ptr<GraphicsTask> draw_grid_mesh;
  ti::NdArray<uint32_t> element_indices_;

  ti::NdArray<float> x_;
  ti::NdArray<float> grid_node_;
  ti::Texture collider_texture_;
  ti::Texture trigger_texture_;
  ti::NdArray<float> grid_lines_;
  ti::NdArray<uint32_t> grid_line_ibo_;
  ...
  virtual void initialize() override final {
    ...
    S2Kinematics kinematics = make_kinematics(
        vec2(0.5, 0.8), 0.0, vec2(4.0, 0.0), 0.0, S2_MOBILITY_DYNAMIC);
    ...
    grid_node_ = runtime.allocate_vertex_buffer(config.grid_resolution *
    ...
    grid_lines_ = runtime.allocate_vertex_buffer(
        config.grid_resolution * config.grid_resolution * 2 * 2, 2, true);
    grid_line_ibo_ = runtime.allocate_index_buffer(
        config.grid_resolution * config.grid_resolution * 2 * 2, 1, true);

    // Assembly grid nodes
    std::vector<glm::vec2> grid_node_host;
    for (int i = 0; i < config.grid_resolution; ++i) {
      for (int j = 0; j < config.grid_resolution; ++j) {
        float dx = 1.0f / (float)config.grid_resolution;
        grid_node_host.push_back(glm::vec2(i, j) * dx);
      }
    }
    grid_node_.write(grid_node_host);

    // Assembly grid lines
    std::vector<glm::vec2> grid_lines_host;
    for (int i = 0; i < config.grid_resolution; ++i) {
      for (int j = 0; j < config.grid_resolution; ++j) {
        float dx = 1.0f / (float)config.grid_resolution;
        grid_lines_host.push_back(glm::vec2(i, j) * dx);
        grid_lines_host.push_back(glm::vec2(i + 1, j) * dx);
        grid_lines_host.push_back(glm::vec2(i + 1, j) * dx);
        grid_lines_host.push_back(glm::vec2(i + 1, j + 1) * dx);
      }
    }
    grid_lines_.write(grid_lines_host);

    std::vector<uint32_t> line_indices;
    for (size_t i = 0; i < config.grid_resolution * config.grid_resolution * 2;
         ++i) {
      line_indices.push_back(2 * i);
      line_indices.push_back(2 * i + 1);
    }
    ...
    draw_mesh =
        runtime
            ...
            .build();
    draw_grid_mesh = runtime
                         .draw_mesh(grid_lines_, grid_line_ibo_,
                                    glm::vec2(offset.x, offset.y),
                                    glm::vec2(extent.x, extent.y))
                         .model2world(model2world)
                         .world2view(world2view)
                         .color(glm::vec3(1.0, 1.0, 1.0))
                         .topo(L_PRIMITIVE_TOPOLOGY_LINE_LIST)
                         .build();
...
std::unique_ptr<App> create_app() {
  return std::unique_ptr<App>(new BackgroundGrid);
}
