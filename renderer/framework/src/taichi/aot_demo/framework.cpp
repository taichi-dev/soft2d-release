...
Framework::Framework(const AppConfig &app_cfg, TiArch arch, bool debug,
                     std::string aot_path)
    : aot_path(aot_path) {
  renderer_ = std::make_unique<Renderer>(debug, app_cfg.framebuffer_width,
...
GraphicsRuntime::GraphicsRuntime(const std::shared_ptr<Renderer> &renderer)
    : ti::Runtime(create_runtime_helper(TI_ARCH_VULKAN, *renderer)),
      renderer_(renderer) {}

ti::NdArray<float> GraphicsRuntime::allocate_vertex_buffer(
    uint32_t vertex_count, uint32_t vertex_component_count, bool host_access) {
...
ti::NdArray<uint32_t> GraphicsRuntime::allocate_index_buffer(
    uint32_t index_count, uint32_t index_component_count, bool host_access) {
  TiMemoryAllocateInfo mai{};
