...
struct AppConfig {
  ...
  uint32_t framebuffer_height = 800;
};
...
struct App {
  ...
  virtual void handle_window_event(GLFWwindow *window) {}
};
...
public:
  ...
  Framework(Framework &&b)
      : aot_path(std::move(b.aot_path)), runtime_(std::move(b.runtime_)),
        renderer_(std::move(b.renderer_)), asset_mgr_(std::move(b.asset_mgr_)),
        frame_(std::exchange(b.frame_, 0)), tic0_(std::move(b.tic0_)),
        tic_(std::move(b.tic_)), toc_(std::move(b.toc_)) {}
  ~Framework();

  Framework &operator=(Framework &&b) {
    aot_path = std::move(b.aot_path);
  ...
  constexpr uint32_t frame() const { return frame_; }
  ...
  constexpr double fps() const { return 1.0 / dt(); }
  ...
  inline AssetManager &asset_mgr() { return *asset_mgr_; }
  // You usually need this in `initialize` and `update`.
  inline GraphicsRuntime &runtime() { return runtime_; }
