...
struct CustomBody : public App {
  ...
  virtual void initialize() override final {
    ...
    // A custom body made of 4 particles
    auto custom_particles = std::vector<S2Vec2>{
        {0.0, 0.00}, {0.00, 0.01}, {0.01, 0.01}, {0.01, 0.0}};
    create_custom_body_from_vector(
        world, make_material(S2_MATERIAL_TYPE_ELASTIC, 1000.0f, 0.3f, 0.2f),
        make_kinematics({0.4f, 0.5f}, 0.0f, {}, 0.0f, S2_MOBILITY_DYNAMIC),
        custom_particles);

    // A rope-like custom body
    custom_particles = {};
    for (int i = -20; i < 20; ++i) {
      custom_particles.push_back({0.005f * i, 0.005f * i});
    }
    create_custom_body_from_vector(
        world, make_material(S2_MATERIAL_TYPE_ELASTIC, 1000.0f, 0.3f, 0.2f),
        make_kinematics({0.6f, 0.5f}, 0.0f, {}, 0.0f, S2_MOBILITY_DYNAMIC),
        custom_particles);

    create_collider(world, make_kinematics({0.6f, 0.3f}),
...
std::unique_ptr<App> create_app() {
  return std::unique_ptr<App>(new CustomBody);
}
